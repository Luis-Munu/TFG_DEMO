...
utils.manage_css_style("Calculadora", "💸", "wide", "collapsed")
...
try:
    property_price = st.slider("Precio de compra", 0.01, 500000.0, 36500.0, 200.0)
    column1, column2= st.columns(2)
    with column1:
        field1, field2 = st.columns(2)
        salary = field1.number_input("Salario", 0.01, 100000.0, float(st.session_state["salary"]), 50.0)
    ...
    furnishing_cost =  field2.slider("Gasto de puesta a punto", 0.01, 30000.0, 0.01, 100.0)
    ...
    with column1:
        ...
        buy_expenses = building_cost + furnishing_cost + property_price * (1.0085 + itp)
        ...
        quota = mortgage_quota + (expenses / 12) + (property_price * ibi / 12)
    ...
    with column3:
        ...
        net_profit = rent * 12 - mortgage_quota * 12 - expenses - property_price * ibi
